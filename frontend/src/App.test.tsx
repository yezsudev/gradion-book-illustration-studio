...
test('navigates to project detail after creating a project', async () => {
  vi.stubGlobal('fetch', vi.fn((url: string, init?: RequestInit) => {
    if (url === '/api/session') return Promise.resolve({ ok: true, status: 200, json: async () => ({ name: 'Mira Hassan', email: 'mira@example.com' }) });
    if (url === '/api/projects' && init?.method === 'POST') return Promise.resolve({ ok: true, status: 201, json: async () => ({ id: 'project-1', title: 'River Book', createdAt: '2026-08-12T00:00:00Z', status: 'Draft', completedSteps: 0, totalSteps: 5, bookText: 'A complete book.', steps: pipelineSteps('PENDING', 'PENDING', 'PENDING', 'PENDING', 'PENDING') }) });
    if (url === '/api/projects') return Promise.resolve({ ok: true, status: 200, json: async () => [] });
    if (url === '/api/health') return Promise.resolve({ ok: true, status: 200, json: async () => ({ status: 'ok' }) });
    return Promise.reject(new Error(`Unexpected request: ${url}`));
  }));
...
test('shows the server-selected current step action', async () => {
  vi.stubGlobal('fetch', projectFetch({
    steps: pipelineSteps('PENDING', 'PENDING', 'PENDING', 'PENDING', 'PENDING'),
  }));
...
test('shows the specific running step while the server reports it', async () => {
  vi.stubGlobal('fetch', projectFetch({
    steps: pipelineSteps('RUNNING', 'PENDING', 'PENDING', 'PENDING', 'PENDING'),
    status: 'In progress',
  }));
...
test('shows a failed step error and its retry action', async () => {
  vi.stubGlobal('fetch', projectFetch({
    steps: pipelineSteps('FAILED', 'PENDING', 'PENDING', 'PENDING', 'PENDING', 'The fake pipeline executor failed.'),
  }));

  render(<App />);
  await openProject();

  expect(screen.getByRole('alert')).toHaveTextContent('The fake pipeline executor failed.');
...
test('shows completed progression and the next step action', async () => {
  vi.stubGlobal('fetch', projectFetch({
    status: 'In progress',
    completedSteps: 1,
    steps: pipelineSteps('COMPLETED', 'PENDING', 'PENDING', 'PENDING', 'PENDING'),
  }));
  ...
  expect(await screen.findByRole('button', { name: 'Run Characters' })).toBeInTheDocument();
});

test('keeps project detail metadata after a run response updates pipeline state', async () => {
  vi.stubGlobal('fetch', projectFetch({
    steps: pipelineSteps('PENDING', 'PENDING', 'PENDING', 'PENDING', 'PENDING'),
    runResponse: { status: 'In progress', completedSteps: 1, steps: pipelineSteps('COMPLETED', 'PENDING', 'PENDING', 'PENDING', 'PENDING') },
  }));
...
test('shows a spinner while a step request is still running', async () => {
  ...
  expect(await screen.findByRole('button', { name: /Style is running/ })).toBeDisabled();
...
test('opens the project detail named by the URL after refresh', async () => {
  ...
  expect(await screen.findByRole('heading', { name: 'River Book' })).toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Run Characters' })).toBeInTheDocument();
  window.history.replaceState({}, '', '/');
});
...
function projectFetch(project: Record<string, unknown>) {
  return vi.fn((url: string, init?: RequestInit) => {
    if (url === '/api/session') return Promise.resolve({ ok: true, status: 200, json: async () => ({ name: 'Mira Hassan', email: 'mira@example.com' }) });
    if (url === '/api/projects') return Promise.resolve({ ok: true, status: 200, json: async () => [{ id: 'project-1', title: 'River Book', createdAt: '2026-08-12T00:00:00Z', status: project.status || 'Draft', completedSteps: project.completedSteps || 0, totalSteps: 5 }] });
    if (url === '/api/projects/project-1') return Promise.resolve({ ok: true, status: 200, json: async () => ({ id: 'project-1', title: 'River Book', createdAt: '2026-08-12T00:00:00Z', status: project.status || 'Draft', completedSteps: project.completedSteps || 0, totalSteps: 5, bookText: 'A complete book.', ...project }) });
    if (url === '/api/projects/project-1/steps/STYLE/run' && init?.method === 'POST') return Promise.resolve({ ok: true, status: 200, json: async () => project.runResponse });
    if (url === '/api/health') return Promise.resolve({ ok: true, status: 200, json: async () => ({ status: 'ok' }) });
    return Promise.reject(new Error(`Unexpected request: ${url}`));
  });
}
...
function pipelineSteps(first: string, second: string, third: string, fourth: string, fifth: string, error?: string) {
  return [
    { key: 'STYLE', state: first, canRun: first === 'PENDING', canRetry: first === 'FAILED', canRecover: false, error: error || null },
    { key: 'CHARACTERS', state: second, canRun: second === 'PENDING' && first === 'COMPLETED', canRetry: second === 'FAILED', canRecover: false, error: null },
    { key: 'PORTRAITS', state: third, canRun: false, canRetry: false, canRecover: false, error: null },
    { key: 'CHAPTERS', state: fourth, canRun: false, canRetry: false, canRecover: false, error: null },
    { key: 'ILLUSTRATIONS', state: fifth, canRun: false, canRetry: false, canRecover: false, error: null },
  ];
}
