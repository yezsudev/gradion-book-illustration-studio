...
export function getSession(): Promise<Identity> {
...
export function runStep(
  projectId: string,
  step: ProjectStep["key"],
  style?: string,
): Promise<ProjectPipeline> {
  return request<ProjectPipeline>(`/projects/${projectId}/steps/${step}/run`, {
    method: "POST",
    ...(step === "STYLE" && style?.trim()
      ? {
          headers: { "Content-Type": "application/json" },
...
export function recoverStep(
  ...
  return request<ProjectPipeline>(
